...
type Pendiente = {
  ...
  fecha: string;
  hora: string;
  user: { id: string; nombre: string; apellido: string; telefono: string };
};
type AlumnaHorario = {
  id: string;
  nombre: string;
    ...
    <div style={{ ...card, marginBottom: 16, padding: 0, overflow: "hidden" }}>
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontWeight: 700, fontSize: 14, color: palette.ink }}>
            {titulo}
          </span>
          {contador !== undefined && contador > 0 && (
            <span
              style={{
                fontSize: 11.5,
                fontWeight: 700,
                color: contadorColor ?? palette.moss,
                background: (contadorColor ?? palette.moss) + "22",
                padding: "2px 8px",
                borderRadius: 999,
              }}
            >
...
export default function AdminDashboard() {
  ...
  const toggle = (clave: keyof typeof abiertas) =>
  ...
  const irAReservasDe = (id: string, nombreCompleto: string) =>
    router.push(
      `/admin/reservas?userId=${id}&nombre=${encodeURIComponent(nombreCompleto)}`,
    );

  if (!datos)
    return (
      <p style={{ color: palette.inkSoft, textAlign: "center", padding: 40 }}>
        Cargando…
      </p>
    );
    ...
    <div>
      <div style={{ marginBottom: 20 }}>
        ...
        <p style={{ color: palette.inkSoft, fontSize: 14, margin: 0 }}>
          Resumen general de Monte Pilates
        </p>
      </div>

      <Seccion
        titulo="Pendientes de pago"
        contador={datos.pendientesDePago.length}
        contadorColor={palette.clayDark}
        abierta={abiertas.pendientes}
        onToggle={() => toggle("pendientes")}
      >
        {datos.pendientesDePago.length === 0 ? (
          <p style={{ fontSize: 13, color: palette.inkSoft, margin: 0 }}>
            No hay ninguna clase pendiente de pago.
          </p>
        ) : (
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button
                    ...
                    style={{
                      ...
                      gap: 6,
                      background: "none",
                      border: `1.5px solid ${palette.moss}`,
                      color: palette.moss,
                      fontWeight: 700,
                      fontSize: 12.5,
                      borderRadius: 8,
      ...
      <Seccion
        titulo={`Hoy · ${fmtDia(datos.hoy.fecha, { weekday: "long", day: "numeric", month: "long" })}`}
        abierta={abiertas.hoy}
        onToggle={() => toggle("hoy")}
      >
        {datos.hoy.bloqueado ? (
          <p style={{ fontSize: 13, color: palette.inkSoft, margin: 0 }}>
            Hoy está bloqueado — no hay clases.
          </p>
        ) : datos.hoy.horarios.length === 0 ? (
          <p style={{ fontSize: 13, color: palette.inkSoft, margin: 0 }}>
            Hoy no hay franja horaria configurada.
          </p>
        ) : (
      ...
      <Seccion
        titulo="Horarios libres esta semana"
        abierta={abiertas.semana}
        onToggle={() => toggle("semana")}
      >
        {datos.semana.length === 0 ? (
          <p style={{ fontSize: 13, color: palette.inkSoft, margin: 0 }}>
            No hay más días de estudio esta semana.
          </p>
        ) : (
      ...
      <Seccion
        titulo="Alumnas por mes"
        abierta={abiertas.alumnosMes}
        onToggle={() => toggle("alumnosMes")}
      >
        ...
        <p
          style={{ fontSize: 11.5, color: palette.inkSoft, margin: "10px 0 0" }}
        >
          Cuenta alumnas distintas con al menos una clase reservada (no
          canceladas) en cada mes.
        </p>
      </Seccion>
    </div>
  );
